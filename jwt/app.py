...
from serilizers import serialize
import json
load_dotenv()
...
def login():
    try:
        ...
        if user:
            try:
                # token should expire after 100 min
...
def add_book(current_user):
    try:
        ...
        if not book:
            return {
                "message": "Invalid data, you need to give the book title, description, image_url, user_id,",
                "data": None,
                "error": "Bad Request"
            }, 400
        if not request.files["image_url"]:
            return {
                "message": "cover image is required",
                "data": None
            }, 400

        book["image_url"] = request.host_url+"static/books/"+save_pic(request.files["image_url"])
